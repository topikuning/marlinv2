...
MIGRATION_SQL = [
    ...
    # BOQ Lifecycle — Addendum approval chain (KPA) + god-mode audit
    "ALTER TABLE contract_addenda ADD COLUMN IF NOT EXISTS kpa_approved_by_id UUID",
    "ALTER TABLE contract_addenda ADD COLUMN IF NOT EXISTS kpa_approved_at TIMESTAMP",
    "ALTER TABLE contract_addenda ADD COLUMN IF NOT EXISTS kpa_approval_notes TEXT",
    "ALTER TABLE contract_addenda ADD COLUMN IF NOT EXISTS signed_by_id UUID",
    "ALTER TABLE contract_addenda ADD COLUMN IF NOT EXISTS signed_at TIMESTAMP",
    "ALTER TABLE contract_addenda ADD COLUMN IF NOT EXISTS god_mode_bypass BOOLEAN NOT NULL DEFAULT FALSE",
    ...
    "CREATE INDEX IF NOT EXISTS idx_field_obs_contract_type ON field_observations (contract_id, type)",
    # Variation Order
    """CREATE TABLE IF NOT EXISTS variation_orders (
        ...
        quantity_calculation TEXT,
        cost_impact NUMERIC(18,2) DEFAULT 0,
        source_observation_id UUID REFERENCES field_observations(id) ON DELETE SET NULL,
        submitted_by_user_id UUID,
        ...
        bundled_addendum_id UUID REFERENCES contract_addenda(id) ON DELETE SET NULL,
        god_mode_bypass BOOLEAN NOT NULL DEFAULT FALSE,
        created_at TIMESTAMP DEFAULT NOW(),
        updated_at TIMESTAMP DEFAULT NOW(),
        CONSTRAINT uq_vo_number_per_contract UNIQUE (contract_id, vo_number)
    )""",
    "CREATE INDEX IF NOT EXISTS idx_vo_contract_status ON variation_orders (contract_id, status)",
    ...
    """CREATE TABLE IF NOT EXISTS variation_order_items (
        ...
        master_work_code VARCHAR(30),
        description TEXT NOT NULL,
        unit VARCHAR(30),
        volume_delta NUMERIC(18,4) DEFAULT 0,
...
ROLES = [
    ...
    ("kpa",       "KPA (Kuasa Pengguna Anggaran)",
     "Menyetujui Addendum kontrak dengan perubahan nilai > 10%", False,
     ["contract.read","contract.update","report.read","payment.read","review.read"],
     ["dashboard","contracts","reports_daily","reports_weekly","scurve",
      "payments","reviews","warnings"]),
